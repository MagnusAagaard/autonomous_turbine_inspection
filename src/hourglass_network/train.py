...
class Trainer:
    ...
    def plot_preds(self, images):
        ...
        for idx in np.arange(images.shape[0]):
            ...
            #NOTE: Changed HERE and indexes start from 0 instead of 3
            #output_img = output[:,:,:3]
            #plt.imshow(output_img[...,::-1])
            plt.imshow(input_img[...,::-1])
            ax.set_title("Output")
            ax = fig.add_subplot(9,images.shape[0],2*images.shape[0]+idx+1,xticks=[],yticks=[])
            plt.imshow(output[:,:,0], cmap='gray', vmin=0, vmax=np.max(output[:,:,0]))
            ax.set_title(f"Tips {np.max(output[:,:,0])}")
            ax = fig.add_subplot(9,images.shape[0],3*images.shape[0]+idx+1,xticks=[],yticks=[])
            plt.imshow(output[:,:,1], cmap='gray', vmin=0, vmax=np.max(output[:,:,1]))
            ax.set_title(f"Center {np.max(output[:,:,1])}")
            ax = fig.add_subplot(9,images.shape[0],4*images.shape[0]+idx+1,xticks=[],yticks=[])
            plt.imshow(output[:,:,2], cmap='gray', vmin=0, vmax=np.max(output[:,:,2]))
            ax.set_title(f"Top {np.max(output[:,:,2])}")
            ...
            ax.set_title(f"Bottom {np.max(output[:,:,3])}")
            ...
            ax.set_title(f"Bot-->Top {np.max(output[:,:,4])}")
            ...
            ax.set_title(f"Top-->WC {np.max(output[:,:,5])}")
            ...
            ax.set_title(f"WC-->Tips {np.max(output[:,:,6])}")
        return fig
